...
struct event {
	...
	u64 bytes_counter; //ADDED BY YUN
};
					    ...
					    struct sk_buff *skb, int reason)
{
	...
	//__u64 packet_len = BPF_CORE_READ(skb, len); // Length of the dropped packet
	//bpf_printk("new skb data_len: %u\n", packet_len);
	//if (packet_len == 0) {
    //	bpf_printk("Packet data_len is zero\n");
	//}
	//if (skb == NULL) {
	//	bpf_printk("skb is NULL\n");
	//}
	event->packet_counter++;
	//event->bytes_counter = packet_len;
