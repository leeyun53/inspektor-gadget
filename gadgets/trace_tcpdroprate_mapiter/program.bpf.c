...
struct traffic_t {
	//size_t sent; //# of bytes sent
	//size_t received; //# of bytes received
	size_t sent_pkts;
	...
	if (skb_val != NULL) {
		...
		*/
		...
		if (!trafficp) {
			...
			//zero.received = 0;
			zero.sent_pkts = 0;
			zero.received_pkts = 0;
			bpf_map_update_elem(&ip_map, &ip_key, &zero, BPF_NOEXIST);
		} else {
			//trafficp->dropped = len;
			trafficp->dropped_pkts = 1;
			//trafficp->sent = 0;
			//trafficp->received = 0;
			trafficp->sent_pkts = 0;
			trafficp->received_pkts = 0;
...
static int probe_ip(bool receiving, struct sock *sk, size_t size)
{
	...
	if (!trafficp) {
		...
		if (receiving) {
			...
			//trafficp->sent = 0;
			trafficp->sent_pkts = 0;
			trafficp->received_pkts = 1;
		}
		else {
			//trafficp->sent += size;
			//trafficp->received = 0;
			trafficp->sent_pkts = 1;
			trafficp->received_pkts = 0;
		}
		//trafficp->dropped = 0;
		trafficp->dropped_pkts = 0;
		bpf_map_update_elem(&ip_map, &ip_key, trafficp, BPF_EXIST);
	}
