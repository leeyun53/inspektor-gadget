...
Inspektor Gadget supports exporting logs using an otlp-grpc exporter. You can log the events of any datasource by
configuring exporters in the `operator.otel-logs` section of the config file like so:
...
If set to true, the gRPC connection will not use TLS encryption.
